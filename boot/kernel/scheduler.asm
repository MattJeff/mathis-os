...
fork:
    ...
    ; Get parent PCB
    mov r8, [current_process]

    ; Find free slot for child
    mov rdi, process_table
    xor ecx, ecx
...
.fork_found_slot:
    ; RDI = child PCB, R8 = parent PCB, ECX = child slot index
    ...
    ; Assign new PID to child
    mov eax, [next_pid]
    inc dword [next_pid]
    mov [rdi + PCB_PID], ax

    ; Set parent PID
    movzx eax, word [r8 + PCB_PID]
    mov [rdi + PCB_PARENT], rax

    ; Allocate new stack for child
    ; Stack address = PROC_STACK_BASE + (slot * PROC_STACK_SIZE)
    mov eax, ecx
    shl eax, 12                             ; * 4096
    add eax, PROC_STACK_BASE
    add eax, PROC_STACK_SIZE                ; Stack grows down
    mov [rdi + PCB_RSP], rax
    sub eax, PROC_STACK_SIZE
    mov [rdi + PCB_STACK_BASE], rax
...
; Output: Does not return on success, RAX = -1 on error
; ════════════════════════════════════════════════════════════════════════════
exec:
    push rbx
    push rcx
    ...
    ; Update process entry point and RIP
    mov [r8 + PCB_ENTRY], rax
    mov [r8 + PCB_RIP], rax

    ; Reset stack
    mov rax, [r8 + PCB_STACK_BASE]
    add rax, PROC_STACK_SIZE
    mov [r8 + PCB_RSP], rax

    ; Clear registers
    mov qword [r8 + PCB_RAX], 0
    mov qword [r8 + PCB_RBX], 0
    mov qword [r8 + PCB_RCX], 0
    mov qword [r8 + PCB_RDX], 0
    mov qword [r8 + PCB_RSI], 0
    mov qword [r8 + PCB_RDI], 0
    mov qword [r8 + PCB_RBP], 0
    mov qword [r8 + PCB_R8], 0
    mov qword [r8 + PCB_R9], 0
    mov qword [r8 + PCB_R10], 0
    mov qword [r8 + PCB_R11], 0
    mov qword [r8 + PCB_R12], 0
    mov qword [r8 + PCB_R13], 0
    mov qword [r8 + PCB_R14], 0
    mov qword [r8 + PCB_R15], 0

    ; Reset time slice
    mov dword [r8 + PCB_TICKS], TIME_SLICE

    ; Force reschedule (will pick up with new entry point)
    cli
    call scheduler_schedule
    ; Should not return
...
; Input: EDI = PID to wait for (-1 = any child)
; Output: EAX = PID of terminated child, -1 on error
;         ECX = exit status (currently always 0)
; ════════════════════════════════════════════════════════════════════════════
waitpid:
    push rbx
    push rdx
    ...
    mov r8d, edi                            ; Save target PID

    ; Get current process PID
    mov rdi, [current_process]
    movzx r9d, word [rdi + PCB_PID]
...
.wait_search:
    ...
    cmp qword [rbx + PCB_PARENT], r9
    jne .wait_next

    ; Check if PID matches (or -1 for any)
    cmp r8d, -1
    je .wait_found
    movzx eax, word [rbx + PCB_PID]
    cmp eax, r8d
...
.wait_found:
    ...
    movzx eax, word [rbx + PCB_PID]         ; Return child PID
    mov byte [rbx + PCB_STATE], PROC_STATE_FREE  ; Free the slot
    xor ecx, ecx                            ; Exit status = 0
    jmp .wait_done
...
.wait_block:
    ...
    mov rdi, [current_process]
    mov byte [rdi + PCB_STATE], PROC_STATE_BLOCKED
...
; Output: EAX = parent PID
; ════════════════════════════════════════════════════════════════════════════
getppid:
    mov rdi, [current_process]
    mov eax, [rdi + PCB_PARENT]
    ret

; ════════════════════════════════════════════════════════════════════════════
; SLEEP - Sleep for specified ticks
; Input: EDI = number of ticks to sleep
; ════════════════════════════════════════════════════════════════════════════
sleep:
    push rbx
    push rcx

    mov rbx, [scheduler_ticks]
    add rbx, rdi                            ; Wake time

.sleep_loop:
    cmp [scheduler_ticks], rbx
    jge .sleep_done
